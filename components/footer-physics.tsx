...
export function FooterPhysics({
  ...
  useEffect(() => {
    ...
    // Add velocity limiter through events instead
    Events.on(mouseConstraint, "mousemove", () => {
      const body = mouseConstraint.body;
      if (body && body.speed > 20) {
    ...
    function createBoundaries(width: number, height: number) {
      return [
        Bodies.rectangle(width / 2, -50, width, 100, { isStatic: true }), // Top - thicker
        Bodies.rectangle(-50, height / 2, 100, height, { isStatic: true }), // Left - thicker
