...
export function Skateboard({
  ...
  const wheelTextures = useTexture(wheelTexturesURLs);
  wheelTextures.forEach((texture) => {
    texture.flipY = false;
    texture.colorSpace = THREE.SRGBColorSpace;
  });
  const wheelTextureIndex = wheelTexturesURLs.findIndex(
    (url) => url === wheelTextureURL
  );
  const wheelTexture = wheelTextures[wheelTextureIndex];

  const deckTextures = useTexture(deckTexturesURLs);
  deckTextures.forEach((texture) => {
    texture.flipY = false;
    texture.colorSpace = THREE.SRGBColorSpace;
  });
  const deckTextureIndex = deckTexturesURLs.findIndex(
